...
<template>
  <main class="workspace" :class="{ 'drawer-open': store.drawerOpen }">
    <Transition name="scene" mode="out-in">
      ...
      <ThreeFarmScene v-else key="three" :active-module="store.activeModule" :active-sub-layer="store.activeSubLayer" :selected-id="store.selectedEntityId" @select="selectEntity" />
    </Transition>

    <FarmTopBar dark @search="showToast(`正在搜索：${$event}`)" @notify="notificationsOpen = !notificationsOpen" />
    <WeatherCard @detail="handleModule('environment')" />
    ...
    <FarmContextDrawer :entity="store.selectedEntity" :open="store.drawerOpen" :module="store.activeModule" @close="store.drawerOpen=false" @select="selectEntity" @action="showToast($event)" />
    <FarmBusinessDock :active="store.activeModule" @change="handleModule" />
    <GreenhouseMonitorModal :open="monitorOpen" :entity="store.selectedEntity" :zone="monitorZone" @close="monitorOpen=false" @action="showToast($event)" />
