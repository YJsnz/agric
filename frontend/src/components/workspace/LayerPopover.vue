...
const items = ref([
  { label: '地块边界', on: true }, { label: '设备标签', on: true }, { label: '摄像头覆盖', on: false },
  { label: '灌溉管网', on: false }, { label: '告警位置', on: true }
])
</script>
<template><Transition name="pop"><div v-if="open" class="layers"><strong>场景图层</strong><label v-for="item in items" :key="item.label"><span>{{ item.label }}</span><input v-model="item.on" type="checkbox" /><i></i></label></div></Transition></template>
