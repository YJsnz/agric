      ...
      <section class="modal">
        ...
        <div class="body">
          <div class="video-stage">
            ...
            <ProgressBar v-if="detectionState === 'loading'" class="detection-progress" :value="null" label="载入 YOLO 检测结果" pending-label="解析帧数据" tone="dark" compact />
            <div
...
.detection-progress{position:absolute;z-index:8;left:50%;top:52%;transform:translate(-50%,-50%);width:min(280px,72%);padding:10px 12px;border:1px solid rgba(255,255,255,.12);border-radius:12px;background:rgba(5,27,21,.72);backdrop-filter:blur(16px)}
</style>
